...
def sequential_forward_selection(data, beatified, excommunicated, target, n_iters):
...
def sequential_backward_selection(data, beatified, excommunicated, target, n_iters):
...
def BiDirectionalSelection(data, target, n_iters, n_features=0):
    sequence = ['f', 'b'] * int((len(data.columns.tolist()) + 1) / 2)
    ...
    for step in sequence:
        ...
        if step == 'f':
            beatified = sequential_forward_selection(data, beatified, excommunicated, target, n_iters)
            #print("beatified: ",beatified)
        elif step == 'b':
            excommunicated = sequential_backward_selection(data, beatified, excommunicated, target, n_iters)
